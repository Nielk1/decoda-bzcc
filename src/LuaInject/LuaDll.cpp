...
void lua_rawgeti_dll(unsigned long api, lua_State *L, int idx, lua_Integer n)
{
...
HMODULE WINAPI LoadLibraryA_intercept(LPCSTR fileName)
{
...
HMODULE WINAPI LoadLibraryW_intercept(LPCWSTR fileName)
{
...
bool LoadLuaFunctions(const std::unordered_map<std::string, DWORD64>& symbols, HANDLE hProcess)
{
	...
	// Only present in Lua 5.3+
	GET_FUNCTION_OPTIONAL(lua_insert);
	GET_FUNCTION_OPTIONAL(lua_rotate);
...
bool SafeReadCheck(const void* ptr, size_t len) {
    __try {
        volatile char dummy;
        for (size_t i = 0; i < len; ++i) {
            dummy = ((const volatile char*)ptr)[i];
        }
...
bool ScanForSignature(DWORD64 start, DWORD64 length, const char* signature)
{
    ...
    for (DWORD64 i = start; i < start + length - signatureLength; ++i)
    {
        ...
        if (!SafeReadCheck(p, signatureLength))
        {
            break;
        }
...
bool ComputeCRC32(const std::string& filePath, uint32_t& crc)
{
    crc = 0xFFFFFFFF;
...
bool LoadFunctionOffsetsFromMap(
    ...
    std::unordered_map<std::string, DWORD64>& symbols)
{
    ...
    while (std::getline(file, line)) {
        if (line.empty()) {
            inTargetSection = false; // End of a section
            continue;
        }
        ...
        // In the correct section, parse function and offset
        std::istringstream iss(line);
        std::string funcName;
        uintptr_t offset;
        if (iss >> funcName >> std::hex >> offset) {
            symbols[funcName] = offset + base;
        }
    }
...
void LoadSymbolsRecursively(std::set<std::string>& loadedModules, std::unordered_map<std::string, DWORD64>& symbols, HANDLE hProcess, HMODULE hModule)
{
    ...
    if (loadedModules.find(moduleName) == loadedModules.end())
    {
        ...
        // try this first
        if (base != 0 && module.SymType != SymNone)
        {
        ...
        bool manualSymbols = false;

        // if the Symbol Type is None or despite a type like Export we got no symbols
        if (result && (module.SymType == SymNone || symbols.empty()))
        {
            ...
            bool fileExists = GetFileExists(pdbFileName);
            ...
            if (!fileExists || !result)
            {
                ...
                uint32_t crc32 = 0;
                bool found = false;
                if (ComputeCRC32(moduleFileName, crc32)) {
                    // Extract just the filename from pdbFileName
                    char mapFileTitle[_MAX_PATH];
                    ReplaceExtension(pdbFileName, "map");
                    GetFileTitle(pdbFileName, mapFileTitle);

                    // Split g_symbolsDirectory by ';'
                    size_t start = 0, end = 0;
                    while ((end = g_symbolsDirectory.find(';', start)) != std::string::npos) {
                        std::string dir = g_symbolsDirectory.substr(start, end - start);
                        if (!dir.empty() && dir.back() != '\\' && dir.back() != '/')
                            dir += "\\";
                        std::string mapPath = dir + mapFileTitle;
                        if (LoadFunctionOffsetsFromMap(mapPath, crc32, base, symbols)) {
                            found = true;
                            manualSymbols = true;
                            break;
                        }
                    ...
                    // Check the last path (or only path if no ';')
                    std::string dir = g_symbolsDirectory.substr(start);
                    if (!found && !dir.empty()) {
        ...
        if (base != 0 && module.SymType == SymPdb && symbols.empty())
        {
            // SymFromName is really slow, so we gather up our own list of the symbols that we
            // can index much faster.
            SymEnumSymbols_dll(hProcess, base, "lua*", GatherSymbolsCallback, reinterpret_cast<PVOID>(&symbols));
        ...
        if (!manualSymbols && g_warnedAboutLua.find(moduleFileName) == g_warnedAboutLua.end())
        {
...
void HookLoadLibrary()
{
    ...
    if (hModuleKernel != NULL)
    {
		...
		LoadLibraryA_dll = (LoadLibraryA_t)HookFunction(GetProcAddress(hModuleKernel, "LoadLibraryA"), LoadLibraryA_intercept);
		LoadLibraryW_dll = (LoadLibraryW_t)HookFunction(GetProcAddress(hModuleKernel, "LoadLibraryW"), LoadLibraryW_intercept);
    }
...
bool InstallLuaHooker(HINSTANCE hInstance, const char* symbolsDirectory)
{
    ...
    {
		char moduleName[_MAX_PATH];
		GetModuleBaseName(hProcess, module.hModule, moduleName, _MAX_PATH);

		std::string moduleString = moduleName;
		transform(moduleString.begin(), moduleString.end(), moduleString.begin(), tolower);
		//if (moduleString.compare("cos.dll") == 0)
		//{
