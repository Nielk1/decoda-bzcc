...
void wxTreeCtrl::SetItemSelectedColour(const wxTreeItemId& item,
  const wxColour& col)
{
  wxCHECK_RET(item.IsOk(), wxT("invalid tree item"));
                    ...
                    case CDDS_ITEMPREPAINT:
                        {
                                 ...
                                 !(tvItemState & TVIS_DROPHILITED) )
                            {
                                ...
                                if ( attr->HasTextColour() )
                                {
                                    ...
                                    //colBack = attr->GetSelectedColour();
                                    //lptvcd->clrTextBk = wxColourToRGB(colBack);
                                }
                            }

                            //If the node is selected, and without focus
                            if (nmcd.uItemState & CDIS_SELECTED && FindFocus() != this)
                            {
                              if (attr->HasSelectedColour())
                              {
                                wxColour colBack;
                                colBack = attr->GetSelectedColour();
                                lptvcd->clrTextBk = wxColourToRGB(colBack);
