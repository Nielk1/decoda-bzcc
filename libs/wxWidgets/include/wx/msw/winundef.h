...
{
#ifdef _UNICODE
  return CreateWindowW((LPCWSTR)lpClassName, (LPCWSTR)lpWndClass, dwStyle, x, y, w, h,
...
inline HMENU LoadMenu(HINSTANCE instance, LPTSTR name)
{
#ifdef _UNICODE
  return LoadMenuW(instance, (LPCWSTR)name);
#else
...
{
#ifdef _UNICODE
  return FindTextW((LPFINDREPLACEW)lpfindreplace);
#else
...
#ifdef LoadIcon
#undef LoadIcon
inline HICON LoadIcon(HINSTANCE hInstance, LPTSTR lpIconName)
{
#ifdef _UNICODE
...
#ifdef LoadBitmap
#undef LoadBitmap
inline HBITMAP LoadBitmap(HINSTANCE hInstance, LPTSTR lpBitmapName)
{
#ifdef _UNICODE
